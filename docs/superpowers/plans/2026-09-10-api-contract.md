...
**Interfaces:**
- Consumes: the Task 4 lint and exchange validation; Task 10's conventions for identity (the `ErrorResponse` shape if it added one, rate-limit responses); `apps/identity/backend/Identity.Module/Endpoints/Auth/AuthorizationManagementEndpoints.cs`, `IdentityControlPlaneEndpoints.cs`, `ScimProtocolEndpoints.cs`, the owner/user/invitation-management handlers in `AuthAccountEndpoints.cs`, and `Services/ScimProtocolService.cs`.
