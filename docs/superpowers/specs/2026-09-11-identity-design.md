...
- **`internal/contracts`** holds the cross-module types:
  - `Permission{Key, Display, Description, Sensitive, Delegable}`
  - `Principal`, the resolved caller: user id, roles, MFA state, flags
  - the `Access` interface: `Authenticate(r) (Principal, bool)` and `Authorize(ctx, Principal, rule) error`
...
- **`internal/module`** holds the `Module` value from parent §3.6:
  ...
  `Deps` carries the pool, config, logger, clock, `mail.Sender`, `secrets.Box`, `contracts.Access` and the rate limiter.
...
- **Access enforcement from the contract.** Each module passes the generated server a `BaseRouter`. It is built by `module.NewRouter(doc, access, limits)` and implements the generated `ServeMux` interface.
  ...
  - It registers the pattern on an inner `http.ServeMux`, wrapped in the order: rate limit → authenticate → authorize → the generated wrapper. This runs before parameter and body decoding, the same order as .NET, where authorization precedes model binding.
  ...
  - Failures write `AuthErrorResponse` 401 `unauthenticated` or 403 `forbidden`, as the .NET cookie events do for every module.
...
- **Proxy trust.** `TRUSTED_PROXY_CIDRS` is the peer allowlist promised in parent §3.11. `X-Forwarded-*` is honoured only when the connecting peer is inside it, for up to `TRUSTED_PROXY_HOPS` hops. Every rate-limit partition keys on the resulting client IP.
...
- **Validation.** One query per request joins the session, the user and the user's roles. It rejects a session that is revoked or expired, a disabled user, or a SCIM-inactive user who is not an Owner. There is no revocation cache: revocation is immediate, where .NET allowed up to 30 s.
- **New sessions and cookies.** Every sign-in creates a new session, so a fixation token never survives. Password login creates a non-persistent cookie. `rememberMe` on `/login/2fa` creates a persistent one (`Max-Age` = the absolute lifetime). TOTP 2FA, passkey login and MFA enable set `mfa_verified_at` on the caller's session. Password-only login never sets it.
- **Revocation.** Every event that rotated the .NET security stamp revokes the affected user's other sessions, and keeps the caller's own session where .NET reissued the caller's cookie. That covers:
  - password change: self, owner-set, and recovery reset (all sessions)
  - MFA disable, setup and owner reset
  - disable and enable
  - owner edits of email, display name or role
  - delegation revoke (the grantee)
...
- **RBAC.** Ported as it is:
  ...
  - the per-role transaction advisory lock (the first 8 bytes of SHA-256 of the role id)
  ...
  - error codes and both 409 codes as .NET: `authorization_conflict` under roles, users and delegations, `concurrency_conflict` under groups, and `role_mapped` as a 403
...
- **Workforce OIDC** uses `coreos/go-oidc` v3.21.0 and `golang.org/x/oauth2` v0.37.0:
  ...
  - authorization code with PKCE and nonce, requested with `response_mode=query` so the SameSite=Lax state cookie reaches the callback. `POST /oidc/callback` is still accepted, as the contract documents.
...
- **SCIM 2.0** is hand-written for the 14 operations.
  - **Token and ingress:**
    ...
    - 120 requests per minute, partitioned by the token when authenticated, else by IP (429 `tooMany`)
    - bodies must be `application/scim+json` (else 415) and at most 256 KiB
  - **Protocol:**
    - correlation by `externalId`, and re-POSTing the same `externalId` replaces the user
    - Owners are immune, and `externalId` is immutable
    - soft delete and `active:false` make the user inactive, which ends their sessions at the next request
...
- `SMTP_HOST`, `SMTP_PORT`, `SMTP_USERNAME`, `SMTP_PASSWORD`, `SMTP_FROM`, `SMTP_TLS` (`implicit` | `starttls` | `none`; `none` only with insecure transport)
- `OIDC_*` (as above)
- `SCIM_TOKEN`, `SCIM_PREVIOUS_TOKEN`, `SCIM_PREVIOUS_TOKEN_EXPIRES_AT`
- `TRUSTED_PROXY_CIDRS`

Every one is validated in `internal/config` with the existing collect-all-problems style.
