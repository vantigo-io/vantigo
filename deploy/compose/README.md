...
The Customers, Products, Energy, Communications and Projects modules are enabled
by default (`MODULES` in `vantigo.env`). They share one PostgreSQL database named
`vantigo`, with independent `identity`, `customers`, `products`, `energy`,
`communications` and `projects` schemas and migration histories.
...
GRANT USAGE ON SCHEMA identity, customers, products, energy, communications, projects TO "vantigo_app";
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA identity, customers, products, energy, communications, projects TO "vantigo_app";
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA identity, customers, products, energy, communications, projects TO "vantigo_app";
```
...
`/api/v1/products`, `/api/v1/energy`, `/api/v1/communications` and
`/api/v1/projects`.
...
See [communications](../../docs/communications.md). The Projects module needs no
environment variable of its own; see [projects](../../docs/projects.md).
