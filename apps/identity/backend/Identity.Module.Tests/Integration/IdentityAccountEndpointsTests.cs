...
public sealed class IdentityAccountEndpointsTests(IdentityApiFactory factory)
{
    ...
    [Fact]
    public async Task DeleteOfScimMappedFederatedUserReturnsConflictAndPreservesHistory()
    {
        ...
        var federation = new FederationConnection
        {
            ...
            DisplayName = $"Delete provenance {Guid.NewGuid():N}",
            Authority = "https://issuer.delete-provenance.integration.test",
            ClientId = "delete-provenance-client",
        ...
        var mapping = new ScimUserMapping
        {
            ...
            UserName = $"delete-provenance-{Guid.NewGuid():N}@integration.test",
            UpstreamActive = true,
        ...
        var federated = new FederatedIdentity
        {
            ConnectionId = federation.Id,
            Issuer = federation.Authority,
            Subject = $"delete-provenance-{Guid.NewGuid():N}",
            DirectoryTenantId = Guid.NewGuid().ToString("D"),
        ...
        await using (var scope = factory.Services.CreateAsyncScope())
        {
            ...
            db.FederatedIdentities.Add(federated);
            await db.SaveChangesAsync();
        }

        using var owner = await factory.CreateOwnerClientAsync();
        var response = await owner.DeleteAsync($"/api/v1/identity/owner/users/{userId}");
        Assert.Equal(HttpStatusCode.Conflict, response.StatusCode);
        var error = await response.Content.ReadFromJsonAsync<ErrorResponse>();
        Assert.Equal("provenance_conflict", error!.Error.Code);

        await using var verifyScope = factory.Services.CreateAsyncScope();
        var verifyDb = verifyScope.ServiceProvider.GetRequiredService<AccountsDbContext>();
        Assert.True(await verifyDb.Users.AnyAsync(item => item.Id == userId));
        Assert.True(await verifyDb.ScimUserMappings.AnyAsync(item => item.Id == mapping.Id));
        Assert.True(await verifyDb.FederatedIdentities.AnyAsync(item => item.Id == federated.Id));
    }
