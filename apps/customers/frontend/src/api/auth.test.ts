...
describe("auth api", () => {
  ...
  it("rotates the CSRF token after a successful login", async () => {
    const fetchMock = stubFetch(
      (url: RequestInfo | URL) => {
        if (String(url) === "/api/v1/identity/login") return Promise.resolve(jsonResponse(200, session));
  ...
  it("treats session and credential 401s as expected control flow", async () => {
    ...
    const fetchMock = stubFetch(
      (url: RequestInfo | URL) => {
        if (String(url) === "/api/v1/identity/antiforgery")
          return Promise.resolve(jsonResponse(200, { token: "login-token" }));
  ...
  it("clears the CSRF token even when sign-out fails", async () => {
    const fetchMock = stubFetch((url: RequestInfo | URL) => {
      if (String(url) === "/api/v1/identity/antiforgery")
        return Promise.resolve(jsonResponse(200, { token: "logout-token" }));
      return Promise.resolve(new Response(null, { status: 500 }));
    });

    await expect(signOut()).rejects.toMatchObject({ status: 500 });
    expect(getCsrfToken()).toBeNull();
    expect(fetchMock.calls.map(([url]) => url)).toEqual(["/api/v1/identity/antiforgery", "/api/v1/identity/logout"]);
  });
});
