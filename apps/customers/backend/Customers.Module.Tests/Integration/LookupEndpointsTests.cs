...
public sealed class LookupEndpointsTests : IDisposable
{
    ...
    {
        var response = await _client.GetFromJsonAsync<LookupResponse>("/api/v1/customers/lookup/brreg?legalId=923609016");
    ...
    public async Task BrregLookup_WhenRegistryReturnsNoMatches_ReturnsEmptyList()
    {
        ...
        var response = await _client.GetFromJsonAsync<LookupResponse>("/api/v1/customers/lookup/brreg?search=no-such-entity");
    ...
    {
        var response = await _client.GetAsync($"/api/v1/customers/lookup/brreg?{queryString}");
    ...
    public async Task BrregLookup_WhenRegistryIsUnavailable_ReturnsBadGateway()
    {
        ...
        var response = await _client.GetAsync("/api/v1/customers/lookup/brreg?search=equinor");
