...
public sealed class Phase3MfaIntegrationTests
{
    ...
    public async Task SetupEnableAndTwoFactorLoginUseTheBuiltInContinuationAndAmrCookie()
    {
        ...
        var completed = await PostJson(client, "/api/v1/identity/login/2fa", new { code = Totp(setupDto.SharedKey!) });
    ...
    public async Task RecoveryCodeIsAcceptedOnceAndOwnerManagementNeedsMfaWhenRequired()
    {
        ...
        Assert.Equal(HttpStatusCode.OK, (await PostJson(client, "/api/v1/identity/login/2fa", new { code = recovery })).StatusCode);

        await PostJson(client, "/api/v1/identity/logout", null);
        Assert.True((await Login(client)).RequiresTwoFactor);
        var replay = await PostJson(client, "/api/v1/identity/login/2fa", new { code = recovery });
    ...
    public async Task BadTotpAttemptsLockOutAndAssistedResetRequiresMfaOwner()
    {
        ...
        {
            last = (await PostJson(client, "/api/v1/identity/login/2fa", new { code = "000000" })).StatusCode;
    ...
    public async Task AssistedResetRequiresMfaOwnerAndInvalidatesTargetSessionAndState()
    {
        ...
        Assert.Equal(HttpStatusCode.OK, (await targetClient.GetAsync("/api/v1/identity/session")).StatusCode);
        ...
        var forbidden = await PostJson(nonMfaOwner, $"/api/v1/identity/owner/mfa/reset/{targetId}", new { });
        Assert.Equal(HttpStatusCode.Forbidden, forbidden.StatusCode);

        var reset = await PostJson(mfaOwner, $"/api/v1/identity/owner/mfa/reset/{targetId}", new { });
        Assert.Equal(HttpStatusCode.OK, reset.StatusCode);
        Assert.Equal(HttpStatusCode.Unauthorized, (await targetClient.GetAsync("/api/v1/identity/session")).StatusCode);
        await using (var scope = factory.Services.GetRequiredService<IServiceScopeFactory>().CreateAsyncScope())
        {
            var users = scope.ServiceProvider.GetRequiredService<UserManager<ApplicationUser>>();
            var db = scope.ServiceProvider.GetRequiredService<AccountsDbContext>();
    ...
    private static async Task<HttpClient> LoginTarget(FreshCustomersApiFactory factory, string email, string password, string authenticatorKey)
    {
        ...
        Assert.True(challenge!.RequiresTwoFactor);
        var completed = await PostJson(client, "/api/v1/identity/login/2fa", new { code = Totp(authenticatorKey) });
        Assert.Equal(HttpStatusCode.OK, completed.StatusCode);
        return client;
    }
    ...
    private static async Task<HttpClient> CreateNonMfaOwnerClient(FreshCustomersApiFactory factory)
    {
        ...
        var client = await factory.CreateAntiforgeryClientAsync();
        Assert.Equal(HttpStatusCode.OK, (await client.PostAsJsonAsync("/api/v1/identity/login", new { email, password })).StatusCode);
        return client;
    }
