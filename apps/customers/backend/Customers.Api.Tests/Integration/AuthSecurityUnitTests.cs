...
public sealed class AuthSecurityUnitTests
{
    ...
    public void CommandLineRequiresExplicitCommandAndPreservesRemainingArguments()
    {
        ...
        Assert.Equal(CustomerApiCommand.NoArguments, CustomerApiCommandLine.Parse([]).Command);
        Assert.Equal(
            CustomerApiCommand.Api,
            CustomerApiCommandLine.Parse(["api", "--urls", "http://localhost"]).Command);
        Assert.Equal(
            ["--urls", "http://localhost"],
            CustomerApiCommandLine.Parse(["api", "--urls", "http://localhost"]).RemainingArguments);
        Assert.Equal(CustomerApiCommand.Migrate, CustomerApiCommandLine.Parse(["migrate"]).Command);
        Assert.Equal(CustomerApiCommand.Seed, CustomerApiCommandLine.Parse(["seed"]).Command);
        Assert.Equal(CustomerApiCommand.Invalid, CustomerApiCommandLine.Parse(["unknown"]).Command);
        Assert.Equal(
            CustomerApiCommand.NoArguments,
            CustomerApiCommandLine.Parse([
                "--environment=Development",
                "--contentRoot=/tmp/customers",
                "--applicationName=Vantigo.Customers.Api",
            ]).Command);
        Assert.Equal(CustomerApiCommand.Invalid, CustomerApiCommandLine.Parse(["--unknown"]).Command);
    }
    ...
    private static IdentityOptions ResolveIdentityOptions(string environmentName)
    {
        ...
        services.AddCustomerIdentity(new TestHostEnvironment(environmentName));
