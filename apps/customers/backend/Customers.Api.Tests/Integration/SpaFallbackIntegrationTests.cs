...
public sealed class SpaFallbackIntegrationTests : IDisposable
{
    ...
    public async Task DeepLink_WithDefaultBasePath_ServesTemplatedIndex()
    {
        ...
        var response = await client.GetAsync("/customers/some/deep/link");
        var html = await response.Content.ReadAsStringAsync();

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        Assert.Equal("text/html", response.Content.Headers.ContentType?.MediaType);
        Assert.Equal("no-cache", response.Headers.CacheControl?.ToString());
        Assert.Contains("/customers/assets/index-abc123.js", html);
    ...
    public async Task Whitelabeling_InjectsConfiguredTitleLogoAndSupport()
    {
        ...
        var html = await client.GetStringAsync("/customers/some/deep/link");
    ...
    public async Task Whitelabeling_DefaultsToTheAppNameWithoutBranding()
    {
        ...
        Assert.Contains("<title>Customers</title>", html);
        Assert.Contains("\"title\":\"Customers\"", html);
    ...
    public async Task ApiAndAuthGuards_StillPrecedeTheSpaFallback()
    {
        ...
        var api = await client.GetAsync("/customers/api/unknown");
        var auth = await client.GetAsync("/customers/auth/unknown");
