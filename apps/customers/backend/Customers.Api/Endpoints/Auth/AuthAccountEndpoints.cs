...
internal static class AuthAccountEndpoints
{
    ...
    {
        var owner = app.MapGroup("/auth/owner")
        ...
        app.MapGet("/auth/invitations/validate", ValidateInvitation)
            .RequireRateLimiting(AuthRateLimitPolicies.InvitationAcceptance);
        app.MapPost("/auth/invitations/accept", AcceptInvitation)
            .RequireAntiforgery()
            .RequireRateLimiting(AuthRateLimitPolicies.InvitationAcceptance);
        app.MapPost("/auth/password-recovery/request", RequestPasswordRecovery)
            .RequireAntiforgery()
            .RequireRateLimiting(AuthRateLimitPolicies.PasswordRecovery);
        app.MapPost("/auth/password-recovery/reset", ResetPassword)
            .RequireAntiforgery()
            .RequireRateLimiting(AuthRateLimitPolicies.PasswordRecovery);
        ...
        CancellationToken cancellationToken)
    {
        ...
        return TypedResults.Created($"/auth/owner/invitations/{invitation.Id}", ToResponse(invitation));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            invitation = await dbContext.Invitations
                .FromSqlInterpolated($"SELECT * FROM accounts.invitations WHERE token_hash = {tokenHash} FOR UPDATE")
        ...
        return TypedResults.Created("/auth/session", new AuthSuccessResponse(
