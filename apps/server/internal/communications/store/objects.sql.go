...
const cleanupRecordCompletedExists = `-- name: CleanupRecordCompletedExists :one
...
func (q *Queries) CleanupRecordCompletedExists(ctx context.Context, storageKey string) (bool, error) {
	row := q.db.QueryRow(ctx, cleanupRecordCompletedExists, storageKey)
	var exists bool
	err := row.Scan(&exists)
	return exists, err
}

const insertPendingCleanupRecord = `-- name: InsertPendingCleanupRecord :exec
...
type InsertPendingCleanupRecordParams struct {
	ID            uuid.UUID
	MessageID     *uuid.UUID
	StorageKey    string
...
func (q *Queries) InsertPendingCleanupRecord(ctx context.Context, arg InsertPendingCleanupRecordParams) error {
	_, err := q.db.Exec(ctx, insertPendingCleanupRecord,
		arg.ID,
		arg.MessageID,
		arg.StorageKey,
		arg.NextAttemptAt,
...
const markCleanupRecordPending = `-- name: MarkCleanupRecordPending :exec
...
type MarkCleanupRecordPendingParams struct {
	MessageID     *uuid.UUID
	NextAttemptAt time.Time
	ID            uuid.UUID
}
...
func (q *Queries) MarkCleanupRecordPending(ctx context.Context, arg MarkCleanupRecordPendingParams) error {
	_, err := q.db.Exec(ctx, markCleanupRecordPending, arg.MessageID, arg.NextAttemptAt, arg.ID)
	return err
}
