...
const projectExpenseGroups = `-- name: ProjectExpenseGroups :many
...
WHERE e.project_id = ANY($1::integer[])
...
type ProjectExpenseGroupsRow struct {
	ProjectID      int32
	Currency       string
	Bucket         string
	LineCount      int64
...
func (q *Queries) ProjectExpenseGroups(ctx context.Context, projectIds []int32) ([]ProjectExpenseGroupsRow, error) {
	rows, err := q.db.Query(ctx, projectExpenseGroups, projectIds)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ProjectExpenseGroupsRow
	for rows.Next() {
		var i ProjectExpenseGroupsRow
		if err := rows.Scan(
			&i.ProjectID,
			&i.Currency,
			&i.Bucket,
