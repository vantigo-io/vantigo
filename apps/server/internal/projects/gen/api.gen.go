...
type ProjectEconomyCost struct {
	...
	// Margin What is left over: the value of the work plus what the expenses will bill, less what the work cost plus what the expenses cost — every term in the project's own currency, added exactly and rounded once at the end rather than half by half. All four terms are **across all three buckets**, the same basis the labour half has always used: the work's own total (approved, submitted and draft together, as `actuals.totalAmount` and `cost.total` report it) and the expenses' total (`expenses.totalCost` and `expenses.totalAmount`), never the approved bucket alone and never two different bases mixed. What is approved and what is not is shown by the three buckets themselves — `actuals.approved/submitted/draft` and `expenses.approved/submitted/draft`. With expenseTracking false the two expense terms are not there and this is the three buckets' bill amount minus their cost, as it has always been. Negative when the project has cost more than it brings in.
...
type ProjectEconomyExpenseCurrency struct {
	...
	// ReadyAmount What of this currency is ready to invoice — approved, billable, priced and not yet invoiced.
...
type ProjectEconomyExpenses struct {
	...
	// ReadyCount How many lines can go on an invoice today: the unit approved, the line billable, a bill amount present and not yet invoiced. Absent when the project carries no currency.
...
type ProjectEconomyReadyAmount struct {
	...
	// TotalAmount amount and expenseAmount together — what is ready to invoice in this currency altogether, which is what the readyAmount sort orders rows by. Absent exactly when expenseTracking is false.
	TotalAmount *float64 `json:"totalAmount,omitempty"`
}
