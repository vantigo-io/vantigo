...
const closeAnonymisedFollowUps = `-- name: CloseAnonymisedFollowUps :exec
UPDATE customers.customers_timeline_entries
SET follow_up_done_at = $1::timestamptz
WHERE customer_id = $2::int AND follow_up_on IS NOT NULL AND follow_up_done_at IS NULL
`

type CloseAnonymisedFollowUpsParams struct {
	Now        time.Time
	CustomerID int32
}
...
func (q *Queries) CloseAnonymisedFollowUps(ctx context.Context, arg CloseAnonymisedFollowUpsParams) error {
	_, err := q.db.Exec(ctx, closeAnonymisedFollowUps, arg.Now, arg.CustomerID)
	return err
}
...
const listTimelineRevisionsForExport = `-- name: ListTimelineRevisionsForExport :many
...
WHERE e.customer_id = $1 AND r.revision_number < e.current_revision
...
func (q *Queries) ListTimelineRevisionsForExport(ctx context.Context, customerID int32) ([]CustomersCustomersTimelineEntriesRevision, error) {
	rows, err := q.db.Query(ctx, listTimelineRevisionsForExport, customerID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
