...
const countCustomerGroupMembers = `-- name: CountCustomerGroupMembers :one
SELECT count(*) FROM customers.customers WHERE group_id = $1
`
...
// not a count.
func (q *Queries) CountCustomerGroupMembers(ctx context.Context, groupID *uuid.UUID) (int64, error) {
	row := q.db.QueryRow(ctx, countCustomerGroupMembers, groupID)
	var count int64
	err := row.Scan(&count)
	return count, err
}

const customerGroupMembership = `-- name: CustomerGroupMembership :one
...
type CustomerGroupMembershipRow struct {
	GroupID                 *uuid.UUID
	GroupName               *string
...
func (q *Queries) CustomerGroupMembership(ctx context.Context, id int32) (CustomerGroupMembershipRow, error) {
	...
	err := row.Scan(&i.GroupID, &i.GroupName, &i.DefaultPaymentTermsDays)
	return i, err
}

const customerGroupsForCustomers = `-- name: CustomerGroupsForCustomers :many
...
type CustomerGroupsForCustomersRow struct {
	CustomerID int32
	ID         uuid.UUID
	Name       string
}
...
func (q *Queries) CustomerGroupsForCustomers(ctx context.Context, customerIds []int32) ([]CustomerGroupsForCustomersRow, error) {
	rows, err := q.db.Query(ctx, customerGroupsForCustomers, customerIds)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []CustomerGroupsForCustomersRow
	for rows.Next() {
		var i CustomerGroupsForCustomersRow
		if err := rows.Scan(&i.CustomerID, &i.ID, &i.Name); err != nil {
			return nil, err
		}
...
DELETE FROM customers.customer_groups WHERE id = $1
`
...
SELECT id, name, default_payment_terms_days FROM customers.customer_groups WHERE id = $1
`
...
func (q *Queries) GetCustomerGroup(ctx context.Context, id uuid.UUID) (GetCustomerGroupRow, error) {
	...
	err := row.Scan(&i.ID, &i.Name, &i.DefaultPaymentTermsDays)
	return i, err
}
...
type InsertCustomerGroupRow struct {
	ID                      uuid.UUID
	Name                    string
	DefaultPaymentTermsDays *int32
}
...
func (q *Queries) InsertCustomerGroup(ctx context.Context, arg InsertCustomerGroupParams) (InsertCustomerGroupRow, error) {
...
func (q *Queries) ListCustomerGroups(ctx context.Context) ([]ListCustomerGroupsRow, error) {
	...
	defer rows.Close()
	var items []ListCustomerGroupsRow
	for rows.Next() {
...
type SetCustomerGroupRow struct {
	ID             int32
...
func (q *Queries) SetCustomerGroup(ctx context.Context, arg SetCustomerGroupParams) (SetCustomerGroupRow, error) {
	row := q.db.QueryRow(ctx, setCustomerGroup,
		arg.GroupID,
		arg.UpdatedAt,
		arg.ID,
		arg.ExpectedRevision,
	)
	var i SetCustomerGroupRow
	err := row.Scan(
		&i.ID,
		&i.CustomerNumber,
		&i.Name,
...
const updateCustomerGroupRow = `-- name: UpdateCustomerGroupRow :one
UPDATE customers.customer_groups
SET name = $1, default_payment_terms_days = $2, updated_at = $3::timestamptz
WHERE customers.customer_groups.id = $4
...
type UpdateCustomerGroupRowParams struct {
	Name                    string
	DefaultPaymentTermsDays *int32
	Now                     time.Time
	ID                      uuid.UUID
...
type UpdateCustomerGroupRowRow struct {
	ID                      uuid.UUID
	Name                    string
	DefaultPaymentTermsDays *int32
	CustomerCount           int64
...
func (q *Queries) UpdateCustomerGroupRow(ctx context.Context, arg UpdateCustomerGroupRowParams) (UpdateCustomerGroupRowRow, error) {
	row := q.db.QueryRow(ctx, updateCustomerGroupRow,
		arg.Name,
		arg.DefaultPaymentTermsDays,
		arg.Now,
		arg.ID,
	)
	var i UpdateCustomerGroupRowRow
	err := row.Scan(
		&i.ID,
		&i.Name,
		&i.DefaultPaymentTermsDays,
