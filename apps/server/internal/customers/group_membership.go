...
func (s *server) PutCustomersByIdGroup(ctx context.Context, req gen.PutCustomersByIdGroupRequestObject) (gen.PutCustomersByIdGroupResponseObject, error) {
	...
	// otherwise, fix the QUERY, not this handler: uuidPtrEqual and deref below
	// both take pointers, and a "" that means "no group" would be a second
	// spelling of the nil the column already has.
