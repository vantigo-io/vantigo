...
describe("AppShellLayout whitelabeling", () => {
  ...
  it("uses the injected title as module name and logo alt text", () => {
    window.__VANTIGO_APP__ = { basePath: "/products/", title: "Acme ERP", support: {} };

    renderShell();

    expect(screen.getByRole("heading", { name: "Acme ERP" })).toBeInTheDocument();
    expect(screen.getByRole("img", { name: "Acme ERP" })).toBeInTheDocument();
  });
  ...
  it("replaces the logo with the injected logo URL", () => {
    ...
    expect(screen.getByRole("img", { name: "Acme ERP" })).toHaveAttribute("src", "https://cdn.acme.test/logo.svg");
  });
