...
const stubSettingsFetch = (invitationResponse: Response) =>
  stubFetch(
    (url: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (method === "GET" && requestUrl === "/auth/owner/invitations") {
        return Promise.resolve(jsonResponse(200, []));
      }
      if (method === "GET" && requestUrl === "/auth/owner/mfa") {
        return Promise.resolve(jsonResponse(200, { twoFactorEnabled: false, mfaEnrollmentRequired: false }));
      }
      if (method === "POST" && requestUrl === "/auth/owner/invitations") return Promise.resolve(invitationResponse);
...
describe("lifecycle form errors", () => {
  ...
  it("shows the setup account-exists message inline, on email, and in a notification", async () => {
    ...
    stubFetch((url: RequestInfo | URL) => {
      if (String(url) === "/auth/bootstrap-status") return Promise.resolve(jsonResponse(200, { available: true }));
      if (String(url) === "/auth/bootstrap") {
