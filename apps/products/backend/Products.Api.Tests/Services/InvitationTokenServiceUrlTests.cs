...
namespace Vantigo.Products.Api.Tests.Services;
...
public sealed class InvitationTokenServiceUrlTests
{
    ...
    public void InvitationUrl_DerivesFromThePublicOriginAndBasePath()
    {
        ...
        Assert.Equal("https://vantigo.example.com/products/invitations/accept?token=raw-token", url);
    }
    ...
    public void PasswordResetUrl_DerivesFromThePublicOriginAndBasePath()
    {
        ...
        Assert.Equal(
            "https://vantigo.example.com/products/password-reset?email=user%40example.test&token=encoded-token",
            url);
    }
