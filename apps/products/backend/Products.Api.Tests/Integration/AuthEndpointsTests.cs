...
public sealed class AuthEndpointsTests
{
    ...
    public AuthEndpointsTests(ProductsApiFactory factory)
    {
        _factory = factory;
    }
    ...
    {
        // App:BasePath defaults to /products; both prefixed and unprefixed
        // requests must work (UsePathBase passes unmatched paths through).
        using var client = _factory.CreateClient();

        var prefixed = await client.GetAsync("/products/auth/antiforgery");
        ...
        var prefixedApi = await client.GetAsync("/products/api/v1/products");
    ...
    public async Task Bootstrap_WithBadSecret_IsRejectedAndRepeatBootstrapIsRejected()
    {
        ...
        var badSecret = await badSecretClient.PostAsJsonAsync("/auth/bootstrap", new
        {
            secret = "wrong-secret",
        ...
        var repeat = await repeatClient.PostAsJsonAsync("/auth/bootstrap", new
        {
    ...
    public async Task BootstrapAndLogin_RequireAntiforgeryToken()
    {
        ...
        var bootstrap = await bootstrapClient.PostAsJsonAsync("/auth/bootstrap", new
        {
            secret = BootstrapSecret,
        ...
        var logout = await authenticatedClient.PostAsync("/auth/logout", content: null);
    ...
    public async Task UnknownApiAndAuthRoutes_Return404InsteadOfSpa()
    {
        ...
        Assert.Equal(HttpStatusCode.NotFound, (await client.GetAsync("/auth/not-a-route")).StatusCode);
    }
    ...
    public async Task Providers_WhenOidcIsNotConfigured_ReturnsNullAndChallengeIsUnavailable()
    {
        ...
        var providers = await client.GetFromJsonAsync<OidcProvidersResponse>("/auth/providers");

        Assert.NotNull(providers);
        Assert.Null(providers!.Oidc);
        Assert.Equal(HttpStatusCode.NotFound, (await client.GetAsync("/auth/oidc/challenge")).StatusCode);
    }
    ...
    public async Task FailedLoginAttempts_TriggerIdentityLockout()
    {
        ...
        {
            using var client = await CreateAntiforgeryClient();
            var response = await client.PostAsJsonAsync("/auth/login", new { email, password = "wrong-password" });
    ...
    private async Task<HttpClient> CreateStandardUserClient()
    {
        ...
        var login = await client.PostAsJsonAsync("/auth/login", new { email, password = "StandardPassword123" });
    ...
    public async Task LoginAndLogout_CanRefreshAntiforgeryToken()
    {
        using var client = _factory.CreateAuthenticatedClient();
        client.DefaultRequestHeaders.Remove("X-XSRF-TOKEN");
        var afterLogin = await client.GetFromJsonAsync<AntiforgeryToken>("/auth/antiforgery");
        client.DefaultRequestHeaders.Add("X-XSRF-TOKEN", afterLogin!.Token);
        Assert.Equal(HttpStatusCode.OK, (await client.PostAsync("/auth/logout", content: null)).StatusCode);

        client.DefaultRequestHeaders.Remove("X-XSRF-TOKEN");
        var afterLogout = await client.GetFromJsonAsync<AntiforgeryToken>("/auth/antiforgery");
...
public sealed class Phase3AccountIntegrationTests
{
    ...
    public Phase3AccountIntegrationTests(ProductsApiFactory factory) => _factory = factory;
    ...
    public async Task InvitationCreatePersistsHashOnlyAndAcceptsOnce()
    {
        ...
        var response = await owner.PostAsJsonAsync("/auth/owner/invitations", new { email, displayName = "Invited", role = "User" });
        ...
        var accepted = await accept.PostAsJsonAsync("/auth/invitations/accept", new { token, password = "InvitedPassword123" });
        Assert.Equal(HttpStatusCode.Created, accepted.StatusCode);
        Assert.Equal(HttpStatusCode.BadRequest,
            (await accept.PostAsJsonAsync("/auth/invitations/accept", new { token, password = "InvitedPassword123" })).StatusCode);
        Assert.Equal(HttpStatusCode.OK, (await accept.GetAsync("/auth/session")).StatusCode);
    }
    ...
    public async Task InvitationDeliveryFailureRevokesAndRecoveryIsGeneric()
    {
        ...
        var response = await owner.PostAsJsonAsync("/auth/owner/invitations", new { email, role = "User" });
    ...
    public async Task InvitationManagementRejectsAnonymousStandardUserAndMissingCsrf()
    {
        ...
        Assert.Equal(HttpStatusCode.Unauthorized, (await anonymous.GetAsync("/auth/owner/invitations")).StatusCode);
        Assert.Equal(HttpStatusCode.Unauthorized,
            (await anonymous.PostAsJsonAsync("/auth/owner/invitations", new { email = "x@example.test", role = "User" })).StatusCode);

        using var standard = await CreateStandardClient();
        Assert.Equal(HttpStatusCode.Forbidden, (await standard.GetAsync("/auth/owner/invitations")).StatusCode);
        using var owner = _factory.CreateAuthenticatedClient();
        owner.DefaultRequestHeaders.Remove("X-XSRF-TOKEN");
        var response = await owner.PostAsJsonAsync("/auth/owner/invitations", new { email = "x@example.test", role = "User" });
        Assert.Equal(HttpStatusCode.BadRequest, response.StatusCode);
        Assert.Equal("csrf_validation_failed", (await response.Content.ReadFromJsonAsync<ErrorResponse>())!.Error.Code);
    }
    ...
    public async Task ResendReplacesTokenAndLeavesOneActiveInvitation()
    {
        ...
        var first = await owner.PostAsJsonAsync("/auth/owner/invitations", new { email, role = "User" });
        ...
        var resend = await owner.PostAsJsonAsync($"/auth/owner/invitations/{firstDto!.Id}/resend", new { });
        ...
        Assert.Equal(HttpStatusCode.BadRequest,
            (await oldAccept.PostAsJsonAsync("/auth/invitations/accept", new { token = firstToken, password = "OldPassword123" })).StatusCode);
        await using var scope = _factory.Services.CreateAsyncScope();
        var db = scope.ServiceProvider.GetRequiredService<AccountsDbContext>();
    ...
    public async Task ConcurrentAcceptanceCreatesExactlyOneAccount()
    {
        ...
        var created = await owner.PostAsJsonAsync("/auth/owner/invitations", new { email, role = "User" });
        ...
        var responses = await Task.WhenAll(clients.Select(client => client.PostAsJsonAsync("/auth/invitations/accept", new { token, password = "ConcurrentPassword123" })));
    ...
    public async Task RecoveryLinkResetsPasswordInvalidatesSessionAndCannotReplay()
    {
        ...
        Assert.Equal(HttpStatusCode.OK,
            (await session.PostAsJsonAsync("/auth/login", new { email, password = originalPassword })).StatusCode);
        await RefreshCsrf(session);
        using var requestClient = await CsrfClient();
        _factory.EmailSender.Clear();
        var known = await requestClient.PostAsJsonAsync("/auth/password-recovery/request", new { email });
        Assert.Equal(HttpStatusCode.OK, known.StatusCode);
        ...
        var resetResponse = await reset.PostAsJsonAsync("/auth/password-recovery/reset", new { email, token, newPassword = changedPassword });
        Assert.Equal(HttpStatusCode.OK, resetResponse.StatusCode);
        Assert.Equal(HttpStatusCode.Unauthorized, (await session.GetAsync("/auth/session")).StatusCode);
        var replay = await reset.PostAsJsonAsync("/auth/password-recovery/reset", new { email, token, newPassword = changedPassword });
        ...
        Assert.Equal(HttpStatusCode.OK, (await login.PostAsJsonAsync("/auth/login", new { email, password = changedPassword })).StatusCode);
    }

    private async Task<HttpResponseMessage> Recovery(string email)
    {
        using var client = await CsrfClient();
        return await client.PostAsJsonAsync("/auth/password-recovery/request", new { email });
    }
    ...
    private async Task<HttpClient> CreateStandardClient()
    {
        ...
        Assert.Equal(HttpStatusCode.OK, (await client.PostAsJsonAsync("/auth/login", new { email, password = "StandardRoutePassword123" })).StatusCode);
        return client;
    }
...
[CollectionDefinition("FreshProductsApi")]
public sealed class FreshProductsApiCollection;

[Collection("FreshProductsApi")]
public sealed class FreshBootstrapTests
{
    [Fact]
    public async Task BootstrapStatus_IsAnonymousJsonAndInitiallyAvailable()
    {
        await using var factory = new FreshProductsApiFactory();
        await factory.StartAsync();
        using var client = factory.CreateCookieClient();

        var response = await client.GetAsync("/auth/bootstrap-status");
        ...
        Assert.DoesNotContain(FreshProductsApiFactory.BootstrapSecret, raw, StringComparison.Ordinal);
    ...
    public async Task BootstrapStatus_IsFalseAfterSuccessfulBootstrap()
    {
        ...
        {
            secret = FreshProductsApiFactory.BootstrapSecret,
    ...
    {
        await using var factory = new FreshProductsApiFactory { ConfigureBootstrapSecret = false };
        await factory.StartAsync();
        ...
        var bootstrap = await csrfClient.PostAsJsonAsync("/auth/bootstrap", new
        {
        ...
        Assert.Equal(HttpStatusCode.Created, bootstrap.StatusCode);
        var consumed = await csrfClient.GetFromJsonAsync<BootstrapStatusDto>("/auth/bootstrap-status");
    ...
    public async Task BadSecretDoesNotChangeFreshState_AndFirstValidBootstrapReturnsCreated()
    {
        ...
        var bad = await client.PostAsJsonAsync("/auth/bootstrap", new
        {
            secret = "wrong-secret",
        ...
        var valid = await client.PostAsJsonAsync("/auth/bootstrap", new
        {
            secret = FreshProductsApiFactory.BootstrapSecret,
            email = "fresh-owner@example.test",
            displayName = "Fresh Owner",
    ...
    public async Task ConcurrentBootstrapCreatesAtMostOneOwnerAndMarker()
    {
        ...
        var responses = await Task.WhenAll(clients.Select(client => client.PostAsJsonAsync("/auth/bootstrap", new
        {
            secret = FreshProductsApiFactory.BootstrapSecret,
