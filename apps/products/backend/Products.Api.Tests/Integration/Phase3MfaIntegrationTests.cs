...
public sealed class Phase3MfaIntegrationTests
{
    ...
    {
        await using var factory = new FreshProductsApiFactory { RequireOwnerMfa = true };
        ...
        var business = await client.GetAsync("/api/v1/products");
    ...
    private static async Task<HttpClient> BootstrapAndLogin(FreshProductsApiFactory factory)
    {
    ...
    private static async Task<HttpClient> LoginTarget(FreshProductsApiFactory factory, string email, string password, string authenticatorKey)
    {
        var client = await factory.CreateAntiforgeryClientAsync();
    ...
    private static async Task<HttpClient> CreateNonMfaOwnerClient(FreshProductsApiFactory factory)
    {
