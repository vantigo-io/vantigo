...
public sealed class MessageResendAndArchiveTests(CommunicationsApiFactory factory)
{
    [Fact]
    public async Task Resend_failed_scope_requeues_only_failed_recipients_and_worker_sends_to_them()
    {
        ...
        var response = await owner.PostAsJsonAsync($"/api/v1/messages/{message.Id}/resend", new { scope = "failed" });
    ...
    public async Task Resend_all_scope_requeues_every_recipient()
    {
        ...
        using var owner = await factory.CreateAuthenticatedClientAsync();
        var response = await owner.PostAsJsonAsync($"/api/v1/messages/{message.Id}/resend", new { scope = "all" });
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
    ...
    public async Task Resend_is_rejected_for_active_jobs_missing_failures_archived_messages_and_bad_scope()
    {
        ...
        var pending = await owner.PostAsJsonAsync($"/api/v1/messages/{pendingMessage.Id}/resend", new { scope = "all" });
        ...
        var noFailures = await owner.PostAsJsonAsync($"/api/v1/messages/{acceptedMessage.Id}/resend", new { scope = "failed" });
        ...
        var badScope = await owner.PostAsJsonAsync($"/api/v1/messages/{acceptedMessage.Id}/resend", new { scope = "everything" });
        ...
        Assert.Equal(HttpStatusCode.OK, (await owner.PostAsync($"/api/v1/messages/{archivedMessage.Id}/archive", null)).StatusCode);
        var archived = await owner.PostAsJsonAsync($"/api/v1/messages/{archivedMessage.Id}/resend", new { scope = "failed" });
        ...
        var missing = await owner.PostAsJsonAsync($"/api/v1/messages/{Guid.NewGuid()}/resend", new { scope = "all" });
    ...
    public async Task Archive_cancels_pending_send_hides_message_from_default_list_and_unarchive_restores_it()
    {
        ...
        using var owner = await factory.CreateAuthenticatedClientAsync();
        var archive = await owner.PostAsync($"/api/v1/messages/{message.Id}/archive", null);
        ...
        Assert.Equal(HttpStatusCode.OK, (await owner.PostAsync($"/api/v1/messages/{message.Id}/archive", null)).StatusCode);

        var visible = (await owner.GetFromJsonAsync<PaginatedMessages>("/api/v1/messages?pageSize=100"))!;
        Assert.DoesNotContain(visible.Data, item => item.Id == message.Id);
        var all = (await owner.GetFromJsonAsync<PaginatedMessages>("/api/v1/messages?pageSize=100&includeArchived=true"))!;
        var listed = Assert.Single(all.Data, item => item.Id == message.Id);
        Assert.NotNull(listed.ArchivedAt);

        var unarchive = await owner.PostAsync($"/api/v1/messages/{message.Id}/unarchive", null);
        ...
        var restored = (await owner.GetFromJsonAsync<PaginatedMessages>("/api/v1/messages?pageSize=100"))!;
    ...
    {
        var mailbox = await db.SharedMailboxes.SingleAsync(item => item.FromAddress == CommunicationsApiFactory.BootstrapMailboxAddress);
